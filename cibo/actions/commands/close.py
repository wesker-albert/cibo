...
class Close(Action):
    ...
    def _door_closes_message(
        self, door_name: str, user_name: str
    ) -> Tuple[Message, Message, Message]:
        """The door is closed by the user."""

        return (
            Message(f"You close {door_name}."),
            Message(f"[cyan]{user_name}[/] closes {door_name}."),
    ...
    def process(self, client: Client, _command: str, args: List[str]) -> None:
        try:
            ...
            door_closes_message = self._door_closes_message(door.name, client.user.name)
